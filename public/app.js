...
const state = {
  ...
  drawer: null,
  role: "COMPANY_OWNER",
  company: "NexFoods Comercio LTDA",
};
...
function login() {
  return el("div", { class: "login" }, [
    el("section", { class: "login-visual" }, [
      el("div", {}, [brand(true)]),
      el("div", {}, [
        el("h1", {}, ["Copiloto financeiro e operacional com IA para PMEs."]),
        el("p", {}, ["Centralize clientes, fornecedores, estoque, vendas, fluxo de caixa, patrimônio e recomendações acionáveis em uma única plataforma multiempresa."]),
      ]),
      el("div", { class: "actions" }, [pill("RBAC + RLS", "info"), pill("LGPD", "ok"), pill("IA explicavel", "warn")]),
    ]),
    el("section", { class: "login-box" }, [
      el("form", { class: "login-card", onsubmit: (e) => { e.preventDefault(); state.authed = true; render(); } }, [
        brand(false),
        el("div", {}, [el("h1", {}, ["Entrar na plataforma"]), el("p", {}, ["Use os dados de demonstração para explorar o MVP frontend."])]),
        field("Email", "email", "gestor@nexfinance.com"),
        field("Senha", "password", "••••••••"),
        el("button", { class: "btn primary", type: "submit" }, ["Entrar"]),
        el("button", { class: "btn", type: "button", onclick: () => { state.authed = true; state.role = "OPERATOR"; render(); } }, ["Ver como operador"]),
      ]),
    ]),
  ]);
}
...
function topbar() {
  return el("header", { class: "topbar" }, [
    ...
    el("div", { class: "top-actions" }, [
      ...
      el("button", { class: "btn icon", onclick: () => openDrawer("Nova ação") }, ["＋"]),
      el("div", { class: "avatar" }, ["EG"]),
    ]),
  ]);
}
